...
# Define variables
DATE=$(date +'%Y.%m.%d')
ARCHIVE_NAME="/var/backups/server/${DATE}_mails.zip"
...
MAILS=(
    "/var/mail/vhosts/emmanuelbeziat.com/contact"
)
...
for MAILS_FILES in "${MAILS[@]}"; do
    rsync "${RSYNC_OPTS[@]}" "$MAILS_FILES" "$TMP_DIR"
done
...
echo "Backup of mails completed on $DATE"
```